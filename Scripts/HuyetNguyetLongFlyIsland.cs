...
public class HuyetNguyetLongFlyIsland : DragonIslandController
{
    ...
    {
        Awakee();
       // scaleduoi = transform.GetChild(1).transform.GetChild(0).transform.Find("Tail").transform.GetChild(0).GetComponent<ParticleSystem>();
    }
    private void Start()
    {
    ...
    protected override void scale(float x)
    {
     ...
     //  Debug.Log("shape.alignToDirection " + shape.alignToDirection);
      //  Vector3 scale = scaleduoi.transform.localScale;
      //  scale.x = Mathf.Abs(scale.x) * x;
       // scaleduoi.transform.localScale = scale;
        base.scale(x);
    }
}
